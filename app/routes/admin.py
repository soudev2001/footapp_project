...
# ============================================================
# SHOP / BOUTIQUE ROUTES
# ============================================================

@admin_bp.route('/shop/products')
@login_required
@role_required('admin')
def shop_products_api():
    """Return products JSON for admin panel"""
    from app.services import get_shop_service, get_user_service
    user_service = get_user_service()
    admin_user = user_service.get_by_id(session.get('user_id'))
    club_id = admin_user.get('club_id')
    shop_service = get_shop_service()
    products = shop_service.get_all_products_admin(club_id)
    from app.models import serialize_docs
    return jsonify({'products': serialize_docs(products)})


@admin_bp.route('/shop/products/add', methods=['POST'])
@login_required
@role_required('admin')
def shop_add_product():
    ...
    if 'image' in request.files and request.files['image'].filename:
        ...
        if ext in allowed:
            ...
            f.save(filepath)
            data['image_url'] = f"/static/uploads/products/{filename}"

    shop_service.create_product(data)
    flash('Produit ajouté avec succès.', 'success')
    return redirect(url_for('admin.dashboard') + '#boutique')


@admin_bp.route('/shop/products/<product_id>/edit', methods=['POST'])
@login_required
@role_required('admin')
def shop_edit_product(product_id):
    ...
    data = {}
    for field in ['name', 'description', 'category', 'sizes']:
    ...
    if 'active' in request.form:
        data['active'] = request.form['active']
    ...
    shop_service.update_product(product_id, data)
    flash('Produit mis à jour.', 'success')
    return redirect(url_for('admin.dashboard') + '#boutique')


@admin_bp.route('/shop/products/<product_id>/delete', methods=['POST'])
@login_required
@role_required('admin')
...
@role_required('admin')
def shop_orders_api():
    """Return orders JSON for admin panel"""
    from app.services import get_shop_service, get_user_service
    user_service = get_user_service()
    shop_service = get_shop_service()
    orders = shop_service.get_all_orders(limit=100)
    # Enrich orders with user info
    for order in orders:
        if order.get('user_id'):
            user = user_service.get_by_id(str(order['user_id']))
            if user:
                order['user_name'] = f"{user.get('first_name', '')} {user.get('last_name', '')}".strip() or user.get('email', '')
    from app.models import serialize_docs
    return jsonify({'orders': serialize_docs(orders)})


@admin_bp.route('/shop/orders/<order_id>/status', methods=['POST'])
@login_required
@role_required('admin')
def shop_update_order_status(order_id):
    ...
    shop_service.update_order_status(order_id, status)
    flash(f'Commande mise à jour: {status}', 'success')
    return redirect(url_for('admin.dashboard') + '#boutique')


@admin_bp.route('/shop/stock-alerts')
@login_required
@role_required('admin')
def shop_stock_alerts():
    from app.services import get_shop_service
    shop_service = get_shop_service()
    alerts = shop_service.get_stock_alerts()
    from app.models import serialize_docs
    return jsonify({'alerts': serialize_docs(alerts)})
