...
class PlayerService:
    ...
    def save_lineup(self, club_id, formation, starters, team_id=None, substitutes=None, name=None, captains=None, set_pieces=None, positions=None):
        ...
        if isinstance(starters, dict):
            starters_data = {pos: ObjectId(pid) for pos, pid in starters.items() if pid}
        elif isinstance(starters, list):
            starters_data = [ObjectId(s) for s in starters if s]
        else:
            starters_data = []
