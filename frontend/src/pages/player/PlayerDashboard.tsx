...
export default function PlayerDashboard() {
  ...
  const statItems = [
    { label: 'Buts', value: stats?.goals ?? profile?.stats?.goals ?? 0, icon: <Target size={20} className="text-pitch-400" /> },
    { label: 'Passes déc.', value: stats?.assists ?? profile?.stats?.assists ?? 0, icon: <Star size={20} className="text-yellow-400" /> },
    { label: 'Matchs', value: stats?.matches_played ?? profile?.stats?.matches_played ?? 0, icon: <Shield size={20} className="text-blue-400" /> },
    { label: 'Note', value: stats?.average_rating ?? '—', icon: <BarChart3 size={20} className="text-purple-400" /> },
  ]
